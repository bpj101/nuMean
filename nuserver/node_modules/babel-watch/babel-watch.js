...
program.option('-e, --extensions [extensions]',
...
program.option('-m, --message [string]',
  'Set custom message displayed on restart (default is ">>> RESTARTING <<<")'
);
...
if (program.ignore != null) ignore = babel.util.arrayify(program.ignore, babel.util
  .regexify);
...
process.on('SIGINT', function() {
  watcher.close();
  // process.exit(1); changing to prevent error on CTL C exit
  process.exit(0);
});
...
function prepareRestart() {
  if (watcherInitialized && childApp) {
    ...
    var restartMessage = program.message ? program.message :
      ">>> RESTARTING <<<";
...
function restartAppInternal() {
  ...
  app.on('message', (data) => {
    ...
    handleFileLoad(filename, (source, sourceMap) => {
      ...
      const mapBuf = new Buffer(sourceMap ? JSON.stringify(sourceMap) :
        0);
      const lenBuf = new Buffer(4);
      try {
        lenBuf.writeUInt32BE(sourceBuf.length, 0);
        fs.writeSync(pipeFd, lenBuf, 0, 4);
        sourceBuf.length && fs.writeSync(pipeFd, sourceBuf, 0,
          sourceBuf.length);
...
function shouldIgnore(filename) {
  if (transpileExtensions.indexOf(path.extname(filename)) < 0) {
    ...
    return path.relative(cwd, filename).split(path.sep).indexOf('node_modules') >=
      0;
  } else {
